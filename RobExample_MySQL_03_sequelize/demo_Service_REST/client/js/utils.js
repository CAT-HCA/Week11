	...
	$.post("http://localhost:3000/users/register", userObj, function() {})
		.done(function(user) {
			...
			if (user.id) {
				...
				users.push(new User(user.id, userObj.userName, userObj.email, userObj.password, userObj.isAdmin));
...
function updateUser(userForm, userObj, usersList) {
	$.ajax({
		...
		success: () => {
			...
			users.push(new User(userObj.id, userObj.userName, userObj.email, userObj.password, userObj.isAdmin));
			displayUsers(usersList);
			...
			if (pseristedUsers.length > 0) {
				...
				for (var indx = 0; indx < pseristedUsers.length; indx++) {
					...
					var newUser = new User(
						pseristedUser.id,
						pseristedUser.user_name,
						pseristedUser.email,
						pseristedUser.password,
						pseristedUser.is_admin
					);
					// push to Array
					users.push(newUser);
				}
